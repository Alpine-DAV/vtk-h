...
class Vtkh(Package, CudaPackage):
    ...
    depends_on("vtk-m@1.5.4~tbb+openmp", when="+openmp")
    depends_on("vtk-m@1.5.4~tbb~openmp", when="~openmp")

    depends_on("vtk-m@1.5.4+cuda~tbb+openmp", when="+cuda+openmp")
    depends_on("vtk-m@1.5.4+cuda~tbb~openmp", when="+cuda~openmp")

    depends_on("vtk-m@1.5.4~tbb+openmp~shared", when="+openmp~shared")
    depends_on("vtk-m@1.5.4~tbb~openmp~shared", when="~openmp~shared")

    depends_on("vtk-m@1.5.4+cuda~tbb+openmp~shared", when="+cuda+openmp~shared")
    depends_on("vtk-m@1.5.4+cuda~tbb~openmp~shared", when="+cuda~openmp~shared")
